# Folk Files

**Stories from the Alvean Federation**, an urban fantasy collection by AJ Reid.

This public repository contains the Astro website and only
publication-approved story and reference content. Draft manuscripts,
worldbuilding notes, reviews, and editorial planning are maintained in a
separate private repository.

## Development

```sh
npm install
npm run dev
npm run build
```

## Rights

Reusable website code is available under [LICENSE-CODE.md](LICENSE-CODE.md).
Fiction, setting material, characters, artwork, branding, and all content
under `src/content/` are excluded from that license and remain
© AJ Reid. All rights reserved. See [RIGHTS.md](RIGHTS.md).
